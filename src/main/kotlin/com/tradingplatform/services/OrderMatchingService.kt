...
class OrderMatchingService {
        ...
        while (SellOrders.isNotEmpty()) {
            ...
            if (potentialSellOrder.price > order.price || order.filledQuantity == order.quantity) {
                ...
                val potentialSellOrderQty =
                    min(order.quantity - order.filledQuantity, potentialSellOrder.quantity - potentialSellOrder.filledQuantity)
    ...
    private fun matchBuyOrder(order: Order){
        val user = order.user
        while(BuyOrders.isNotEmpty()){
            ...
            if(potentialBuyOrder.price < order.price || order.filledQuantity == order.quantity){
                ...
                val potentialBuyOrderQty = min(order.quantity - order.filledQuantity, potentialBuyOrder.quantity - potentialBuyOrder.filledQuantity)
                ...
                if(potentialBuyOrder.filledQuantity < potentialBuyOrder.quantity && potentialBuyOrder.filledQuantity > 0) potentialBuyOrder.status = "partially filled"
                BuyOrders.add(potentialBuyOrder)
